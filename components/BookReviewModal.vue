...
const handleSubmit = async () => {
    ...
    try {
        ...
        const status = ref<'read' | 'reading' | 'to-read'>('to-read')

        const payload = {
            description: description.value,
            rating: rating.value,
            private: isPrivate.value,
            status: status.value, // Ya no necesita type assertion
        };
            ...
            class="relative bg-white dark:bg-gray-900 rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto"
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <UTextarea
                             v-model="description" :placeholder="status === 'read'
                            ? 'Comparte tus pensamientos sobre este libro...'
                            : status === 'reading'
                                ? 'Anota tus impresiones mientras lees...'
                                : 'Anota por qué quieres leer este libro...'" :rows="4" class="w-full" />
                    </div>

                    <!-- Checkbox privado -->
                    <div class="flex items-start space-x-3">
                        <UCheckbox id="private-review" v-model="isPrivate" />
                ...
                <!-- Footer -->
                <div
                    class="flex justify-end gap-3 p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                    <UButton color="neutral" variant="outline" :disabled="isLoading" @click="handleClose">
                        Cancelar
                    </UButton>
                    <UButton
                         color="primary" :loading="isLoading"
                        :disabled="isLoading || (status === 'read' && rating === 0)" @click="handleSubmit">
