...
const router = useRouter() // Se agrega para la navegación
const {
  selections,
  error,
  stats,
...
// --- CORRECCIÓN CLAVE ---
// Ya no usamos useAsyncData. En su lugar, cargamos los datos en el hook onMounted.
// Esto asegura que la función de carga se ejecute cada vez que el componente
// se monte, incluso si el usuario navega desde otra página sin refrescar.
...
const filteredSelections = computed(() => {
  // --- CORRECCIÓN: Acceder al .value de la ref `selections` del store ---
  // Esto es crucial para que el computed sea reactivo a los cambios del store.
...
const columns: TableColumn<Selection>[] = [{
  ...
  cell: ({ row }) => {
    ...
    const items = [{
      ...
      icon: 'i-lucide-eye',
      onSelect() {
        // Mejor usar el router para la navegación en lugar de un console.log
...
async function randomize() {
  if (selections.value && selections.value.length > 0) { // CORRECCIÓN: Acceder al .value
    try {
    ...
    <ClientOnly @load="isClient = true">
      <div class="flex justify-center w-full min-h-screen p-4">
        <div class="w-full max-w-7xl">
          ...
          <template v-else>
            <div
              class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 mb-6 shadow-sm">
              <div class="flex flex-col sm:flex-row gap-4 sm:items-center sm:justify-between">
                ...
                <div class="flex gap-2 flex-wrap items-center">
                  ...
                  <UButton
                    color="neutral" variant="outline" icon="i-lucide-x-circle" label="Limpiar selección"
                    size="sm" @click="clearSelection" />
            ...
            <div 
              v-else-if="filteredSelections.length > 0"
                    ...
                    <div class="flex items-center justify-between">
                      <div class="flex items-center gap-2">
                        ...
                        <span class="text-yellow-500 text-sm">
                          {{ '★'.repeat(selection.userReview?.rating || 0) + '☆'.repeat(5 -
                            (selection.userReview?.rating || 0)) }}
                        </span>
                      </div>
              ...
              <div class="hidden md:block overflow-x-auto">
                <UTable ref="table" :data="filteredSelections" :columns="columns" class="w-full">
                    ...
                    <div class="p-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700">
                      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                        <div>
                          ...
                          <span class="ml-2 text-gray-600 dark:text-gray-400">{{ getPublicationYear(row.original)
                            }}</span>
                        </div>
                        ...
                        <div v-if="row.original.book_id?.isbn">
                          ...
                          <span class="ml-2 text-gray-600 dark:text-gray-400 font-mono">{{ row.original.book_id.isbn
                            }}</span>
                        </div>
                        ...
                        <div v-if="row.original.book_id?.publisher">
                          ...
                          <span class="ml-2 text-gray-600 dark:text-gray-400">{{ row.original.book_id.publisher
                            }}</span>
                        </div>

                        <div v-if="row.original.dateAdded">
                          <span class="font-medium text-gray-700 dark:text-gray-300">Agregado:</span>
                          <span class="ml-2 text-gray-600 dark:text-gray-400">{{ new
                            Date(row.original.dateAdded).toLocaleDateString('es-ES') }}</span>
                        </div>
                      </div>

                      <div 
                        v-if="row.original.userReview?.description && row.original.userReview.description.trim()"
                        class="mt-4">
                        <span class="font-medium text-gray-700 dark:text-gray-300">Reseña:</span>
                        <p class="mt-1 text-gray-600 dark:text-gray-400 italic">{{ row.original.userReview.description
                          }}</p>
                      </div>
                    </div>
                  </template>
